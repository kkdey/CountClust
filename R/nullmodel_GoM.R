...
#'
#' @description Use null models (popular in ecology) to generate randomized matrix
#'              of counts given the observed data matrix, fit the GoM model to these
#'              null matrices and compare the fit on null model data with that on the
#'              observed data. Used for validating the GoM clusters
#'
#' @param counts The counts matrix (N x G): N- the number of samples, G- number of features
#' @param K  The number of clusters to fit
#' @param tol The tolerance of the GoM model fitted
#' @param null.model The type of nullmodel used (similar to the randomizeMatrix() function argument in picante package)
...
nullmodel_GoM <- function(counts,
                          ...
                          plot=TRUE)
{
    bf_gom_rand <- unlist(lapply(1:iter_randomized,
                  function(n)
                  {
                    rand_counts <- picante::randomizeMatrix(counts,
                                      null.model=null.model, iterations=iter_fill);
                    suppressMessages(topics_rand <- FitGoM(rand_counts, K=K, tol=tol));
                    X <- CheckCounts(rand_counts); n <- nrow(X); p <- ncol(X)
    ...
    ll <- list("GoMBF.obs"=bf_gom_obs,
               ...
               "pval"=pval_bf_gom)
    if(plot){
          plot(density(bf_gom_rand), col="blue", main="Log BF density over randomized matrices",
               xlab="Log Bayes facto", ylab="density", xlim=c(min(bf_gom_obs,bf_gom_rand)-100, max(bf_gom_obs,bf_gom_rand)+100))
