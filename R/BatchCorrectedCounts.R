...
#'
#' @description This function first converts counts data to log CPM
...
#' 
#' @param use_parallel If \code{TRUE}, we do a parallel analysis over
#'   features, else serial application.
#'
#' @return Returns a counts data. with same dimension as the input
...
BatchCorrectedCounts <- function(data, batch_lab, use_parallel = TRUE)
{
    ...
    if(use_parallel){
        batch_removed_counts_mean <-
            do.call(cbind,
                    mclapply(1:dim(trans_data)[2], function(g)
                    {
